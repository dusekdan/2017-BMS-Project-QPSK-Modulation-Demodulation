...
void QPSKModulation(std::istream* dataStream, int &startTick, SndfileHandle handle)
{
    ...
    while (dataStream->get(c0) && dataStream->get(c1))
    {
        ...
        if (value0 == 0 && value1 == 0)
        {
            ...
            {
                buffer [i] = (double) cos((FREQ * 2 * startTick * M_PI) + (M_PI/4));
                startTick++;
            }
        }
        else if (value0 == 0 && value1 == 1)
        {
            for (int i = 0; i < SAMPLING_SHIFT; i++)
            {
                buffer [i] = (double) cos((FREQ * 2 * startTick * M_PI) + (7*M_PI/4));
                startTick++;
            }
        }
        else if (value0 == 1 && value1 == 0)
        {
            for (int i = 0; i < SAMPLING_SHIFT; i++)
            {
                buffer [i] = (double) cos((FREQ * 2 * startTick * M_PI) + (3*M_PI/4));    
                startTick++;
            }
        }
        else
        {
            for (int i = 0; i < SAMPLING_SHIFT; i++)
            {
                buffer [i] = (double) cos((FREQ * 2 * startTick * M_PI) + (5*M_PI/4));
                startTick++;
            }
         }
